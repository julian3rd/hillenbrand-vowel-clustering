...
hillenbrand.kmeans.ratio.data <- 
  read.csv(file.path(data.dir, "hillenbrand-kmeans-formant-ratio-data.csv"), header = TRUE)

hillenbrand.gaussmm.ratio.data <- 
  read.csv(file.path(data.dir, "hillenbrand-gaussmm-formant-ratio-data.csv"), header = TRUE)

ggplot(hillenbrand.kmeans.ratio.data, aes(x = PC1, y = PC2, label = Vowel)) + 
...
ggplot(hillenbrand.gaussmm.ratio.data, aes(x = PC1, y = PC2, label = Vowel)) + 
  geom_text(aes(colour = factor(Label)), size = 5.5, alpha = 0.87) +
  scale_color_brewer(palette = 'Paired') + 
  theme_bw() +
